...
async function fetchLocations(
	...
	const plugins = LSPPlugin.getAll(view);
	const plugin = plugins.find((p) => {
    const caps = p.client.serverCapabilities as Record<string, unknown> | undefined;
    return !!caps?.[CAPABILITY_KEY[kind]];
}) ?? plugins[0];
	if (!plugin) return null;
	const client = plugin.client;
	const capabilities = plugin.client.serverCapabilities as Record<string, unknown> | undefined;
